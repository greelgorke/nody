 ...
 *     .onThreshold( thresholdValue: Number
 *                  , thresholdListener: Function [ array of merge candidates ]
 *                  )
 *        will set the thresholdValue and add a listener for the 'threshold' event.
 * events:
...
Merge.prototype.onThreshhold = function onThreshold( thresholdValue, listener ) {
  this.threshholdValue = thresholdValue

  if ( listener )
    this.on( 'threshold', listener )

  return this
}

function checkThreshold( merge ){
  if ( merge.threshholdValue > 0) {
    ...
    if ( filteredCache.length > merge.threshholdValue ) {
      merge.emit('threshold', filteredCache )
    }
  }
}
